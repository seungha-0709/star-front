  ...
  <tbody class="cart-table">
    <tr v-for="(cartList, index) in cartLists" :key="index" id="selected-color">
      <td v-if="select" class="tb-checkbox">
        <label>
          <input type="checkbox" v-model="select" :value="index" v-on:click="clickCartIndex(index)" />
          <check-button :value="select.includes(index)" />
        </label>
      </td>
      <td class="table-line item-title" :class="{ 'auto-width': select === null }">
      ...
      <td class="table-line amount">
        <button v-if="select" class="amt-btn" v-on:click="amountControl(index, 'min')">
          <minus-icon size="20" class="icon-add minus-icon"></minus-icon>
        </button>
        <p class="amt-price">{{ cartList.amount }}</p>
        <button v-if="select" class="amt-btn" v-on:click="amountControl(index, 'plus')">
