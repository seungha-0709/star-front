<template>
  <div>
    ...
    <modal :modalProps="modalInfoOrder" v-if="modalTypeOnOff[0].onoff" @close="modalOff('order')">
      <modal-order :paymentData="paymentInfo.find((data) => data.id === modalIndex)" />
    </modal>
    ...
    >
      <modal-receipt :paymentData="paymentInfo.find((data) => data.id === modalIndex)" />
    </modal>

    <modal :modalProps="modalInfoCancel" v-if="modalTypeOnOff[2].onoff" @close="modalOff('cancel')">
      <modal-cancel :paymentData="paymentInfo.find((data) => data.id === modalIndex)" />
    </modal>

    <modal :modalProps="modalInfoQna" v-if="modalTypeOnOff[3].onoff" @close="modalOff('qna')">
      <modal-qna :paymentData="paymentInfo.find((data) => data.id === modalIndex)" />
    </modal>
  </div>
</template>
